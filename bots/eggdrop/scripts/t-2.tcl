...
set t2(G21) {
%g
0,2 Estadísticas de %n: 8 Puntos (respuestas) %d%w%mTotal: 0 %e 
...
%3 Your Stats: %4 Points (answers) %d%w%mTotal Ever: %5 %e 
  ...
  set t2(G91) "8Hoy: 0 #%rd %pd (%qd) 8 "
  set t2(G92) ""
  set t2(G93) "Esta Semana: 0 #%rw %pw (%qw) 8 "
  set t2(G94) ""
  set t2(G95) "Este Mes: 0 #%rm %pm (%qm) 8 "
  set t2(G96) ""
  set t2(G97) "#%re %pe (%qe)"
  set t2(G98) "none"
...
set t2(A01) {%1 BogusTrivia iniciará en %x segundos, prepárense!!! }
...
set t2(P01) {1,8 Hoy TOP 10 - %x}

## Top 11-20 players taday ###
set t2(P02) {1,8 Hoy TOP 11-20 - %x}

## Top daily players above 20 ###
set t2(P03) {1,8 Hoy TOP %s-%e - %x}

## Top daily players - stats for each nick ###
set t2(P04) {1,8 #%r: 0,12 %n %p }

## Top 10 players this week ###
set t2(P11) {1,8 Esta Semana TOP 10 - %x}

## Top 11-20 players this week ###
set t2(P12) {1,8 Esta Semana TOP 11-20 - %x}

## Top weekly players above 20 ###
set t2(P13) {1,8 Esta Semana TOP %s-%e - %x}

## Top weekly players - stats for each nick ###
set t2(P14) {1,8 #%r: 0,12 %n %p }

## Top 10 players this month ###
set t2(P21) {1,8 Este Mes TOP 10 - %x}

## Top 11-20 players this month ###
set t2(P22) {1,8 Este Mes TOP 11-20 - %x}

## Top monthly players above 20 ###
set t2(P23) {1,8 Este Mes TOP %s-%e - %x}

## Top monthly players - stats for each nick ###
set t2(P24) {1,8 #%r: 0,12 %n %p }

## Top 10 players ever ###
set t2(P31) {1,8 TOP 10 Jugadores - %x}

## Top 11-20 players ever ###
set t2(P32) {1,8 TOP Jugadores 11-20 - %x}

## Top players ever above 20 ###
set t2(P33) {1,8 TOP Jugadores %s-%e - %x}

## Top players ever - stats for each nick ###
set t2(P34) {1,8 #%r: 0,12 %n %p }
...
## !info reply text ###
set t2(P45) {
%3 %s v%v de %a (%o) - Editado por honewt
%1 El fichero del usuario fue creado el %u (Hay un total de %t jugadores). 
%1 %q Preguntas & %k Kaos en la base de datos (%z preguntas en total). 
%1 Tiempo actual del juego/servidor: %f%c%i. 
}
set t2(P46) {}

## !mystats reply to known player ###
set t2(P50) {0,2 Tu Estadisticas %n: 8 Puntos (contestadas) %d%w%m8Total: 0 %e }

## !mystats reply to unknown player (set empty to not reply to unknown player) ###
set t2(P51) {}

  set t2(P52) "8Hoy: 0 #%rd %pd (%qd) "
  set t2(P53) ""
  set t2(P54) "8Esta Semana: 0 #%rw %pw (%qw) "
  set t2(P55) ""
  set t2(P56) "8Este Mes: 0 #%rm %pm (%qm) "
  set t2(P57) ""
  set t2(P58) "#%re %pe (%qe)"
  set t2(P59) "none"

## !stats <nick> : reply for known player ###
set t2(P60) {0,2 Estadisticas de %n: 8Puntos (contestadas) %d%w%m8Total: 0 %e }

## !stats <nick> : reply for unknown player (set empty to not reply for unknown player) ###
set t2(P61) {}

  set t2(P62) "8Hoy: 0 #%rd %pd (%qd) "
  set t2(P63) ""
  set t2(P64) "8Esta Semana: 0 #%rw %pw (%qw) "
  set t2(P65) ""
  set t2(P66) "8Este Mes: 0 #%rm %pm (%qm) "
  set t2(P67) ""
  set t2(P68) "#%re %pe (%qe)"
  set t2(P69) "none"
...
set t2(P70) {
%2 Uso incorrecto del comando %3%m%2. Usted %3debe%2 proporcionar un Nick.
%2 El uso correcto para este comando es: %3 %m Nick  
}
...
proc TDoNum { {num Error} } {
 switch -exact -- $num {
  ...
  15 {  return Quince  }
  16 {  return Dieciséis  }
  17 {  return Diecisiete  }
  18 {  return Dieciocho  }
  19 {  return Diecinueve  }
  20 {  return Veinte  }
  default {  return $num  }
 }
}
 ...
 if {$wat>"0"} {
   ...
   if {$wat=="1"} {  set uc $lincnt  ;  set t2(-ufilcnt) $uc
     ...
     if {$doda=="1" && $dals!=""} {  set count 0
      ...
      set toplin "1,8 Hoy TOP 10 - "
      if {$t2(today)>"1" && [llength $topls]>"11"} {
        set topln2 "1,8 Hoy TOP 11-20 - "  }
      if {$t2(today)>"2" && [llength $topls]>"21"} {
        set topln3 "1,8 Hoy TOP 21-30 - "  }
      foreach topusr $topls {  incr count
       foreach {tp tc nk hn uh} $topusr { break }
       set tnick [string range $nk 3 end]
       if {$count<"11"} {
         append toplin "1,8 #$count: 0,12 $tnick $tp "
       } elseif {$count<"21"} {
         if {$topln2!=""} {
           append topln2 "1,8 #$count: 0,12 $tnick $tp "  }
       } elseif {$count<"31"} {
         if {$topln3!=""} {
           append topln3 "1,8 #$count: 0,12 $tnick $tp "  }
       }
      ...
      set toplin "1,8 Esta Semana TOP 10 - "
      if {$t2(today)>"1" && [llength $topls]>"11"} {
        set topln2 "1,8 Esta Semana TOP 11-20 - "  }
      if {$t2(today)>"2" && [llength $topls]>"21"} {
        set topln3 "1,8 Esta Semana TOP 21-30 - "  }
      foreach topusr $topls {  incr count
       foreach {tp tc nk hn uh} $topusr { break }
       set tnick [string range $nk 3 end]
       if {$count<"11"} {
      ...
      set toplin "1,8 Este Mes TOP 10 - "
      if {$t2(today)>"1" && [llength $topls]>"11"} {
        set topln2 "1,8 Este Mes TOP 11-20 - "  }
      if {$t2(today)>"2" && [llength $topls]>"21"} {
        set topln3 "1,8 Este Mes TOP 21-30 - "  }
      foreach topusr $topls {  incr count
       foreach {tp tc nk hn uh} $topusr { break }
       set tnick [string range $nk 3 end]
       if {$count<"11"} {
      ...
      set toplin "1,8 TOP 10 Jugadores - "
      if {$t2(today)>"1" && [llength $topls]>"11"} {
          set topln2 "1,8 TOP 11-20 Jugadores - "  }
      if {$t2(today)>"2" && [llength $topls]>"21"} {
          set topln3 "1,8 TOP 21-30 Jugadores - "  }
      foreach topusr $topls {  incr count
       foreach {tp tc nk hn uh} $topusr { break }
       set tnick [string range $nk 3 end]
       if {$count<"11"} {
...
proc TOnOff {nk uh hn ch tx {from 0} } {  global t2 tclr botnick nick
 ...
 if {$t2(-ison)=="0"} {
   ...
   if {$t2(-qfcnt)=="0"} {
    putserv "PRIVMSG $ch :$t2(script): No hay preguntas agregadas."  ;  return 0
   }
   if {$t2(limit)>"0"} {  set t2(-lchek) 1  }
   ...
   } else {  putquick "PRIVMSG $ch :$t2(script) Cargando..."  }
   set t2(-htime) $t2(qtime)  ;  set t2(-ptime) $t2(ptime)  ;  set t2(-stime) $t2(stime)
   ...
   putquick "PRIVMSG $ch :$t2(script) Apagado."
 }
}
...
proc TGotIt {nk uh hn ch tx} {  global t2 tclr botnick  ;  set iskaos 0
 ...
 if {$iskaos=="1"} {  set temp -1  ;  set fnd 0
   ...
   putquick "PRIVMSG $ch :$tclr(-ktu) $nk gana $t2(-points) Puntos por: $tclr(-ktu2) $tx "
   ...
   set say "$tclr(-tu2) Muy Bien,$tclr(-tu1) $nk $tclr(-tu2)La respuesta es =>$tclr(-tu1) $tx $tclr(-tu2)<= Respondiste en$tclr(-tu1)"
   if {$points=="1"} {  set p point  } else {  set p points  }
   putquick "PRIVMSG $ch :$say $qestime $tclr(-tu2)segundos, y ganas$tclr(-tu1) $points $tclr(-tu2)Puntos "
   ...
   set pstr "$tclr(-tu1) $nk $tclr(-tu2)has acertado$tclr(-tu1) $anscnt $tclr(-tu2)seguidas!"
 }
 if {$endnow=="1"} {
   if {$iskaos=="1"} {  set opt 1
     set kalltx "$tclr(-ktu) Enhorabuena, $tclr(-ktu2)los has adivinado todos!!"
     if {[llength $newaddls]>"5" && $t2(-kbon)>"0"} {
       append kalltx " $tclr(-kbon) Todos obtienen un $tclr(-kbon)BONUS de$tclr(-kbon2) $t2(-kbon) Puntos !! "
       set newaddl2 ""
   ...
   foreach {anik ahan aush apnt acnt} $newaddls {
    if {$iskaos=="1"} {
      ...
      if {$fnd>"0"} {  set end [expr {$nidx+7}]  ;  set opt 0
        ...
        putquick "PRIVMSG $ch :$pstr Total Puntos de Hoy:$tclr(-tu1) $da $tclr(-tu2) Esta Semana:$tclr(-tu1) $we $tclr(-tu2) & Este Mes:$tclr(-tu1) $mo "
   ...
   if {$iskaos=="1"} {
     ...
     set kstats "$tclr(-kstat) Total Otorgado:$tclr(-ksta2) $t2(-kgotpnts) Puntos "
     if {[llength $t2(-kusrls)]>"5"} {
       append kstats "$tclr(-kstat)a [expr {[llength $t2(-kusrls)]/5}] Jugadores "
     }
...
proc TDoHint {nk uh hn ch tx} {  global t2 tclr botnick  ;  TBind h u
 if {$t2(-hntnum)=="1"} {
   putserv "PRIVMSG $ch :12 Pista: $t2(-uhint) " ; set t2(-uhint) ""
 } elseif {$t2(-hntnum)=="2"} {
   putserv "PRIVMSG $ch :12 Pista: $t2(-uhnt2) " ; set t2(-uhnt2) ""
 } else {
   putserv "PRIVMSG $ch :12 Pista: $t2(-uhnt3) " ; set t2(-uhnt3) "" }
 array set t2 {-utrigd 1 -udead 0}
 set t2(-htime) $t2(qtime)  ;  set t2(-ptime) $t2(ptime)  ;  set t2(-stime) $t2(stime)
}
 ...
 if {$t2(-hntnum)>=$t2(hntcnt) || $t2(-hntnum)=="-1" || $t2(-hntnum)=="-2"} {  set istop 0
   if {$t2(-hntnum)=="-1"} {
     ...
     if {$iskaos=="1"} {  set kleft [expr {[llength $t2(-kansls)]/4}]
       ...
       if {$t2(givkaos)!="0"} {  set ktutmp "$tclr(-ktu) Se acabo el Tiempo! $tclr(-ktu2) Ninguno Menciono $tclr(-kng) "
         foreach {kans hn2 hn3 kvar} $t2(-kansls) {  append ktutmp "\[$kans\] "  }
       } else {  set ktutmp "$tclr(-ktu2) Times up! "  }
       putquick "PRIVMSG $t2(chan) :$ktutmp"
       if {$kgot>"0"} {
         set ktutm2 "$tclr(-kstat) Correctamente Contestadas:$tclr(-ksta2) $kgot de $t2(-kanstot) "
         if {$t2(-kgotpnts)=="1"} {  set point Point  } else {  set point Points  }
         append ktutm2 "$tclr(-kstat) Total Otorgado:$tclr(-ksta2) $t2(-kgotpnts) Puntos "
         if {$kusrs>"1"} {  append ktutm2 "$tclr(-kstat)a $kusrs Jugadores "  }
         putquick "PRIVMSG $t2(chan) :$ktutm2"
       }
     } else {  set ansr ""
       if {$t2(givansr)!="0"} { set ansr "$tclr(-tu2) La respuesta era -> $tclr(-tu1) $t2(-answer) $tclr(-tu2) <- " }
       putquick "PRIVMSG $t2(chan) :$tclr(-tu1) Se acabo el Tiempo! $ansr"
     }
   }
  ...
  if {$iskaos=="1"} {  set sec $t2(ktime)  ;  set doshort 0
    ...
    set t2(-hnt2) "2,0 2nd Pista: "
    if {$t2(kstyle)>"1" || $doshort=="1"} {  set kleft [expr {[llength $t2(-kansls)]/4}]
      if {$kleft==$t2(-kanstot)} {  append t2(-hnt2) " Todas las respuestas restantes a "
      ...
      } else {  append t2(-hnt2) "$t2(-points) Puntos cada una $t2(-endclr) $tclr(-kpnt1) "  }
      putquick "PRIVMSG $t2(chan) :$t2(-hnt2)[expr {$sec*2}] segundos "
    } else {
      if {$t2(descend)=="1"} {  set t2(-points) [expr {round($t2(-points)/2.0)}]  }
      foreach {kans hn2 hn3 kvar} $t2(-kansls) {  append t2(-hnt2) " $hn2"  }
      append t2(-hnt2) " $t2(-endclr) $tclr(-kpnt1) [expr {$sec*2}] segundos "
      putquick "PRIVMSG $t2(chan) :$t2(-hnt2)"
    }
  } else {  set t2(-qtimer) [utimer $t2(-htime) [list TShoTriv 2]]
    set t2(-hnt2) "2,0 2nd Pista: $t2(-hnt2) $t2(-endclr) "
    if {$t2(descend)=="1"} {  set givpnt [expr {round($t2(-points)/2.0)}]
      if {$givpnt=="1"} {  set point Point  } else {  set point Points  }
      append t2(-hnt2) "$tclr(-pnt1) [expr {$t2(-htime)*2}] segundos &$tclr(-pnt2) $givpnt"
      putquick "PRIVMSG $t2(chan) :$t2(-hnt2) Puntos $tclr(-pnt1)Restantes "
  ...
  if {$iskaos=="1"} {  set sec $t2(ktime)  ;  set doshort 0
    ...
    set t2(-hnt3) "2,0 3ra Pista: "
    if {$t2(kstyle)>"2" || $doshort=="1"} {  set kleft [expr {[llength $t2(-kansls)]/4}]
      if {$kleft==$t2(-kanstot)} {  append t2(-hnt3) " Todas las respuestas restantes a "
      ...
      } else {  append t2(-hnt3) "$t2(-points) Puntos cada una $t2(-endclr) $tclr(-kpnt1) "  }
      putquick "PRIVMSG $t2(chan) :$t2(-hnt3)$sec segundos "
    } else {
      if {$t2(descend)=="1"} {  set t2(-points) [expr {round($t2(-points)/2.0)}]  }
      foreach {kans hn2 hn3 kvar} $t2(-kansls) {  append t2(-hnt3) " $hn3"  }
      append t2(-hnt3) " $t2(-endclr) $tclr(-kpnt1) $sec segundos "
      putquick "PRIVMSG $t2(chan) :$t2(-hnt3)"
    }
  } else {  set t2(-qtimer) [utimer $t2(-htime) [list TShoTriv 3]]
    set t2(-hnt3) "2,0 3ra Pista: $t2(-hnt3) $t2(-endclr) "
    if {$t2(descend)=="1"} {
      set givpnt [expr {round([expr {round($t2(-points)/2.0)}]/2.0)}]
      if {$givpnt=="1"} {  set point Point  } else {  set point Points  }
      append t2(-hnt3) "$tclr(-pnt1) $t2(-htime) segundos &$tclr(-pnt2) $givpnt"
      putquick "PRIVMSG $t2(chan) :$t2(-hnt3) Puntos $tclr(-pnt1)Restantes "
...
proc TReadHist {num {opt -1} } {  global t2 tclr botnick
 ...
 if {$find ne "1+"} {  set lut [lindex $utls 0]  ;  set plls [lindex $plls 0]
   ...
   if {$num=="1"} {  set sholine "1,8 Ayer TOP 10: - "
     if {[llength $plls]>"20"} {  set sholin2 "1,8 Ayer TOP 11-20: - "  }
     if {[llength $plls]>"40"} {  set sholin3 "1,8 Ayer TOP 21-30: - "  }
   } elseif {$num=="2"} {  set sholine "1,8 Semana Pasada TOP 10: - "
     if {[llength $plls]>"20"} {  set sholin2 "1,8 Semana Pasada TOP 11-20: - "  }
     if {[llength $plls]>"40"} {  set sholin3 "1,8 Semana Pasada TOP 21-30: - "  }
   } elseif {$num=="3"} {  set sholine "1,8 Mes Pasado TOP 10: - "
     if {[llength $plls]>"20"} {  set sholin2 "1,8 Mes Pasado TOP 11-20: - "  }
     if {[llength $plls]>"40"} {  set sholin3 "1,8 Mes Pasado TOP 21-30: - "  }
   } elseif {$dwm=="D"} {
   ...
   foreach {nk pt} $plls {  incr cnt
    if {$cnt<"11"} {  append sholine "1,8 #$cnt: 0,12 $nk $pt "
    } elseif {$cnt<"21"} {  append sholin2 "1,8 #$cnt: 0,12 $nk $pt "
    } elseif {[info exists sholin3]} {  append sholin3 "1,8 #$cnt: 0,12 $nk $pt "  }
   }
 } else {
   if {[llength $utls]=="1"} {  return 0  }
   if {$dwm=="D"} {  set sholine "$tclr(-d10) Top Jugadores de: - "
   } elseif {$dwm=="W"} {  set sholine "$tclr(-w10) Top Jugadores de: - "
   } else {  set sholine "$tclr(-m10) Top Jugadores de: - "  }
   set cnt 0
...
proc TMkLines {} {  global t2 tclr botnick nick ;  set t2(-otimer) "" ;  set doques 1 ;  set isbon 0
 ...
 if {$qfline ne ""} {  set qls [split $qfline *] ;  set t2(-allansls) [TStrLo [lrange $qls 1 end]]
  ...
  if {[TStrLo $tqes]=="scramble" || [TStrLo $tqes]=="uword"} {  set temp [TStrLo $t2(-answer)]
   ...
   set t2(-questn) " $tqnum$tq2 ? $t2(-endclr)"
   if {$t2(usrqes)>="1"} {  set t2(-uqes) " $tq2 ? $t2(-endclr)"  }
  } else {
  ...
  if {$t2(-questn) eq ""} {  set t2(-questn) "1,7 $tqnum"
    ...
    if {$ln1cnt<=$t2(maxchar)} {  append t2(-questn) "1,7 $tqes ? $end"
    } else {
  ...
  if {$doques=="1"} {
    ...
    set t2(-hint) "2,0 1er Pista: $t2(-hint) $t2(-endclr) "
    if {$pnttmp=="1"} {  set point Point  } else {  set point Points  }
    if {$isbon=="1"} {
      append t2(-hint) "$tclr(-bonus) BONUS Valor de la Pregunta:$tclr(-bon2) $pnttmp Puntos "
    } else {  append t2(-hint) "$tclr(-pnt1) Valor de la Pregunta:$tclr(-pnt2) $pnttmp Puntos "  }
    set t2(-points) $pnttmp
  } else {
    set t2(-hint) "2,0 1er Pista: [string trim $t2(-hint)] $t2(-endclr) "
    ...
    if {$t2(kstyle)=="0" && $doshort=="0"} {
      ...
      set qlbeg " Valor de la Pregunta:"  ;  set qlend " $pnttmp Puntos "
      set qstrlen [expr {$ln1cnt+[string length $qlbeg]+[string length $qlend]}]
      if {$tclr(-kpnt1) ne ""} {  incr qstrlen [string length $tclr(-kpnt1)]  }
      if {$tclr(-kpnt2) ne ""} {  incr qstrlen [string length $tclr(-kpnt2)]  }
      if {$qstrlen<=$t2(maxchar)} {  append t2(-questn) $tclr(-kpnt1)$qlbeg$tclr(-kpnt2)$qlend
      } else {  append t2(-quesn2) $tclr(-kpnt1)$qlbeg$tclr(-kpnt2)$qlend  }
      set t2(-hint) "2,0 1er Pista: [TDoNum $kanscnt] Posibles Respuestas $t2(-endclr) "
      append t2(-hint) "$tclr(-kpnt1) Cada respuesta Vale:$tclr(-kpnt2) $t2(-points) Puntos "
    }
  }
 }
 if {$t2(-dosave)>"0"} {
 ...
 if {$t2(-shoad)=="1"} {
   ...
   set t2(-preqes) $nerp\x05E\x05E  ;  set t2(-shoad) 0
 } else {
 ...
 if {$opt=="1"} {
   if {$t2(-qfcnt)>"0"} {  return 1
     ...
     if {$havnew==""} {  putserv "PRIVMSG $t2(chan) :$t2(script): No New Question Files Found."
       ...
       putserv "PRIVMSG $t2(chan) :$t2(script): Nuevas preguntas encontradas: [join $havnew]"
       putserv "PRIVMSG $t2(chan) :Usa $tclr(-msg) /msg $botnick .add $tclr(-emsg) para cargar las nuevas preguntas."
     }
     return 0
   }
 }
 ...
 if {[string match -nocase .add $tx] || [string match -nocase .addk $tx]} {  set havnew [TLook]
   if {$havnew!=""} { set jusfls "" ; set juscnt 1 ; set havtot [llength $havnew]
     ...
     if {[string match $t2(myfils) [lindex $jusfls 0]]} {  set frmt $t2(myfmt)
       if {$havtot=="1"} {  putserv "PRIVMSG $nk :\00310Se encontró un archivo BogusQuestion: [join $jusfls]"
...
proc TDoAdd { {mine 0} {othr 0} {iskaos -1} } {  global t2 tclr botnick
   ...
   foreach file [lrange $t2(-newfiles) 2 end] {  incr qfilcnt
    if {$qfilcnt==$curf} {  set error 0  ;  set thiscnt 0  ;  set kaoscnt 0
         ...
         if {$thiscnt=="1" || $thiscnt=="2"} { lappend lastlin $qesedtd
           ...
           if {$error>"0"} {
             ...
             if {$mine=="0"} {
               ...
               } else {  putserv "PRIVMSG $nk :\00310Leer archivo de preguntas: $file"  }
               ...
               if {$t2(-ison)=="1"} { TOnOff $nk $uh $hn $t2(chan) 0 2 ; set wason 1
                 putquick "PRIVMSG $t2(chan) :Game Stopped. Añadiendo Preguntas..."
                 putserv "PRIVMSG $nk :BogusTrivia Game Stopped..."
               } else { set wason 0 }
               if {$wson=="-"} { set wson $wason }
               if {$mine=="1"} {
                 if {$curf>"1"} {  putserv "PRIVMSG $nk : "  }
                 putserv "PRIVMSG $nk :\00310Leer archivo de preguntas: $file"
               }
      ...
      if {$thiscnt=="-1"} {
        ...
        if {$kaoscnt=="0"} {
          putserv "PRIVMSG $nk :Agregando $qadded nuevas preguntas a la base de datos..."
        } else {
          if {$kaoscnt==$qadded} {
            putserv "PRIVMSG $nk :Agregando $qadded new KAOS to the database..."
          } else {  set tqa [expr {$qadded-$kaoscnt}]
            putserv "PRIVMSG $nk :Agregando $tqa preguntas & $kaoscnt KAOS a la base de datos..."
          }
 ...
 if {$curf<$totf} {
   ...
   if {$t2(-ktotal)=="0"} {
     putserv "PRIVMSG $nk :$tottmp $t2(-qtotal) preguntas ahora en la base de datos."
   } elseif {$t2(-qtotal)=="0"} {
...
proc TMix {nk uh hn tx {times 0} {wason 0} {todo 0} } {  global t2 tclr botnick
 ...
 if {$qfills=="" && $kfills==""} {
   putserv "PRIVMSG $nk :No hay preguntas en la base de datos."
   putserv "PRIVMSG $nk :Escribe: .add  para cargar preguntas."  ;  return 0  }
